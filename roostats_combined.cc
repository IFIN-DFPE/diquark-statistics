...
*/
void point_exclusion(DataPoint point, SignalUncertainties uncrt, TFile* output_file) {
    ...
    */
    ...
    // Create dataset       
    mu.setVal(1.);
    RooDataSet* toyData = sb_full.generate(*sbModel->GetObservables(), RooFit::ExpectedData());
    ...
    HypoTestResult* result = asympCalc->GetHypoTest();
    std::cout << "\n p-value for background-only hypothesis: " << result->NullPValue() << "\n";
...
*/
void roostats_combined(const char* configFilePath = nullptr) {
    ...
    for(int i = 0; i < data.size(); i++) {
        point_exclusion(data[i], uncrt_data[i], output_file);
    }
